...
exports = module.exports = function parse(str, options) {
  ...
  return (
    parseNearbyDays(str, options.now) ||
...
function parseLastThisNext(string, now) {
  ...
  if (
    ...
    var dayAbbreviationsRu = DAY_NAMES_RU.map(function (name) {
      return name.substr(0, tokens[1].length)
    })
...
function parseAgoFrom(string, now) {
  var tokens = string.split(/[,\s]+/)
  if (
    ['day', 'days', 'week', 'weeks'].indexOf(tokens[1]) >= 0 &&
    tokens[0].match(NUMBER) &&
    ['ago', 'from'].indexOf(tokens[2]) >= 0
  ) {
    if (tokens[2] === 'ago') {
      if (['day', 'days'].indexOf(tokens[1]) >= 0)
        return addDays(now, tokens[0] * -1)
      if (['week', 'weeks'].indexOf(tokens[1]) >= 0)
        return addDays(now, tokens[0] * 7 * -1)
    }
    if (tokens[2] === 'from') {
      if (['day', 'days'].indexOf(tokens[1]) >= 0)
        return addDays(now, tokens[0])
      if (['week', 'weeks'].indexOf(tokens[1]) >= 0)
        return addDays(now, tokens[0] * 7)
    }
    return null
  } else {
    return null
...
function parseNumberDate(str, usa) {
  ...
  if (match) {
    return usa
      ? date(+match[3], match[1] - 1, +match[2])
      : date(+match[3], match[2] - 1, +match[1])
  } else {
    return null
  }
}
...
function parseNumberDateShortYear(str, usa, cutoff) {
  var match = usa
    ? NUMBER_DATE_SHORT_YEAR_USA.exec(str)
    : NUMBER_DATE_SHORT_YEAR.exec(str)
  if (match) {
    var year = +match[3]
    if (year > cutoff) year += 1900
    else year += 2000
    return usa
      ? date(year, match[1] - 1, +match[2])
      : date(year, match[2] - 1, +match[1])
  } else {
    return null
  }
}
...
function parseNumberDateNoYear(str, usa, today) {
  var match = usa
    ? NUMBER_DATE_NO_YEAR_USA.exec(str)
    : NUMBER_DATE_NO_YEAR.exec(str)
  if (match) {
...
function parseWordyDateParts(rawDay, rawMonth, rawYear, today, cutoff) {
  ...
  if (rawYear) year = rawYear.match(NUMBER) ? rawYear * 1 : null
...
function monthFromName(month) {
  ...
  var monthAbbreviationsRu = MONTH_NAMES_RU.map(function (name) {
    return name.substr(0, month.length)
  })
